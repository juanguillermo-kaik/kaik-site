...
const principles = [
  { label: 'Inicio', title: 'Claridad estratégica', desc: 'Definimos foco, mensaje y estructura para alinear comunicación y objetivos.', icon: '+' },
  { label: 'Desarrollo', title: 'Sistema de contenido', desc: 'Construimos piezas y flujos que sostienen atención, coherencia y ejecución en el tiempo.', icon: '◩' },
  { label: 'Deployment', title: 'Activación digital', desc: 'Implementamos plataformas y experiencias que conectan equipos y optimizan la operación.', icon: '×' },
]
          ...
          <div className="mt-8 grid gap-5 md:grid-cols-2">
            {services.map((item, index) => (
              ...
              >
                ...
                <p className={`mt-4 max-w-[34rem] text-[0.98rem] leading-7 ${index === 1 ? 'text-white/84' : 'text-[#6c7280]'}`}>{item.desc}</p>
                <div className={`mt-5 h-px w-full ${index === 1 ? 'bg-white/24' : 'bg-[linear-gradient(90deg,rgba(0,55,255,0.5),rgba(0,55,255,0.06))]'}`} />
                <div className="mt-5 grid gap-3">
                  {item.bullets.map((bullet) => (
                    <div
                      key={bullet}
                      className={`rounded-[1.05rem] border px-4 py-3 text-[0.94rem] ${index === 1 ? 'border-white/18 bg-white/8 text-white/88' : 'border-white/82 bg-white/60 text-[#505665] backdrop-blur-xl'}`}
                    >
                      {bullet}
                    </div>
                  ))}
