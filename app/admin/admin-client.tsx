  ...
  return <main className="min-h-screen bg-[#f6f8fc] text-[#5e6474]">
    ...
    <div className="md:pl-20">
      ...
      <div className="bg-[radial-gradient(circle_at_90%_100%,rgba(193,219,255,.46),transparent_23rem),linear-gradient(110deg,#eefbfc,#f6f8fc_55%)] px-6 py-7 md:px-10">
        ...
        {view === "posts" ? <PostsView posts={posts} form={form} editingId={editingId} creating={creating} busy={busy} edit={edit} newPost={newPost} closeEditor={closeEditor} save={save} set={set} /> : <AccessView admins={admins} email={adminEmail} setEmail={setAdminEmail} add={addAdministrator} revoke={removeAdministrator} busy={busy} />}
      </div>
    </div>
  </main>;
}
...
function PostsView({ posts, form, editingId, creating, busy, edit, newPost, closeEditor, save, set }: { posts: Post[]; form: Form; editingId: number | null; creating: boolean; busy: boolean; edit: (post: Post) => void; newPost: () => void; closeEditor: () => void; save: (event: React.FormEvent) => void; set: (key: keyof Form, value: string) => void }) {
