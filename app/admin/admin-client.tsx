...
type Post = { id: number; title: string; slug: string; excerpt: string; body: string; category: string; author: string; cover_image: string | null; cover_alt: string | null; og_image: string | null; seo_title: string | null; seo_description: string | null; status: "draft" | "published"; published_at: string | null; updated_at: string };
type Form = Omit<Post, "id" | "published_at" | "updated_at">;
type AdminUser = { id: number; email: string; role: "owner" | "admin" };
type View = "posts" | "access";

const emptyPost: Form = { title: "", slug: "", excerpt: "", body: "", category: "KAIK", author: "KAIK", cover_image: "", cover_alt: "", og_image: "", seo_title: "", seo_description: "", status: "draft" };
...
export default function AdminClient({ initialAuthenticated, initialPosts, initialAdmins, canManageUsers, googleError }: { initialAuthenticated: boolean; initialPosts: Post[]; initialAdmins: AdminUser[]; canManageUsers: boolean; googleError?: string }) {
  ...
  const edit = (post: Post) => { setEditingId(post.id); setCreating(true); setForm({ title: post.title, slug: post.slug, excerpt: post.excerpt, body: post.body, category: post.category, author: post.author, cover_image: post.cover_image, cover_alt: post.cover_alt, og_image: post.og_image, seo_title: post.seo_title, seo_description: post.seo_description, status: post.status }); window.scrollTo({ top: 0, behavior: "smooth" }); };
  const newPost = () => { setEditingId(null); setCreating(true); setForm(emptyPost); };
  const closeEditor = () => { setEditingId(null); setCreating(false); setForm(emptyPost); };
  ...
  async function uploadCover(file: File) {
    ...
    try {
      ...
      const data = await response.json().catch(() => ({}));
      if (!response.ok) return setMessage(data.message || "No se pudo cargar la imagen.");
      setForm((current) => ({ ...current, cover_image: data.url, og_image: current.og_image || data.url, cover_alt: current.cover_alt || file.name.replace(/\.[^.]+$/, "").replace(/[-_]+/g, " ") }));
      setMessage("Imagen cargada correctamente.");
    } catch { setMessage("No fue posible cargar la imagen."); }
    ...
    <div className="md:pl-20">
      ...
      <div className="bg-[radial-gradient(circle_at_90%_100%,rgba(193,219,255,.46),transparent_23rem),linear-gradient(110deg,#eefbfc,#f6f8fc_55%)] px-6 py-7 md:px-10">
        <div className="flex flex-wrap items-center gap-2">
          <Tab active={view === "posts"} onClick={() => setView("posts")}>Blog</Tab>
          {canManageUsers && <Tab active={view === "access"} onClick={() => setView("access")}>Accesos</Tab>}
          {view === "posts" && <Tab active={creating} onClick={newPost}>Crear</Tab>}
        </div>
        ...
        {view === "posts" ? <PostsView posts={posts} form={form} editingId={editingId} creating={creating} busy={busy} uploading={uploading} edit={edit} newPost={newPost} closeEditor={closeEditor} save={save} set={set} uploadCover={uploadCover} /> : <AccessView admins={admins} email={adminEmail} setEmail={setAdminEmail} add={addAdministrator} revoke={removeAdministrator} busy={busy} />}
      </div>
    </div>
  </main>;
}
...
function PostsView({ posts, form, editingId, creating, busy, uploading, edit, newPost, closeEditor, save, set, uploadCover }: { posts: Post[]; form: Form; editingId: number | null; creating: boolean; busy: boolean; uploading: boolean; edit: (post: Post) => void; newPost: () => void; closeEditor: () => void; save: (event: React.FormEvent) => void; set: (key: keyof Form, value: string) => void; uploadCover: (file: File) => void }) {
  const editing = creating;
  const [previewOpen, setPreviewOpen] = useState(false);
  if (editing) return <><form onSubmit={save} className="mt-7 rounded-2xl border border-[#e0e6ef] bg-white p-6 shadow-[0_4px_16px_rgba(61,91,138,.11)] md:p-8"><div className="flex items-center justify-between gap-4"><div><h2 className="text-2xl font-semibold text-[#1371fa]">{editingId ? "Editar entrada" : "Crear entrada"}</h2><p className="mt-1 text-sm">Completa los campos para guardar un borrador o publicar.</p></div><div className="flex gap-2"><button type="button" onClick={() => setPreviewOpen(true)} className="rounded-full border border-[#cbd8ec] bg-white px-5 py-2.5 text-sm font-semibold text-[#1371fa]">Vista previa</button><button type="button" onClick={closeEditor} className="rounded-full bg-[#eef1f5] px-5 py-2.5 text-sm font-semibold text-[#737a89]">Cancelar</button></div></div><div className="mt-7 grid gap-5 md:grid-cols-2"><Field label="Título"><input value={form.title} onChange={(event) => set("title", event.target.value)} required /></Field><Field label="Categoría"><input value={form.category} onChange={(event) => set("category", event.target.value)} /></Field><Field label="Autor"><input value={form.author} onChange={(event) => set("author", event.target.value)} /></Field><Field label="URL / slug"><input value={form.slug} onChange={(event) => set("slug", event.target.value)} placeholder="se-genera-desde-el-titulo" /></Field><Field label="Estado"><select value={form.status} onChange={(event) => set("status", event.target.value)}><option value="draft">Borrador</option><option value="published">Publicado</option></select></Field></div><Field label="Bajada"><textarea value={form.excerpt} onChange={(event) => set("excerpt", event.target.value)} rows={3} /></Field><div className="mt-5"><span className="text-sm font-medium text-[#697282]">Contenido</span><RichTextEditor value={form.body} onChange={(value) => set("body", value)} /></div><details className="mt-6 rounded-xl bg-[#f5f8ff] p-5"><summary className="cursor-pointer font-semibold text-[#626a79]">SEO y portada</summary><div className="mt-5 grid gap-5 md:grid-cols-2"><Field label="Cargar imagen de portada"><input type="file" accept="image/jpeg,image/png,image/webp,image/avif" disabled={uploading} onChange={(event) => { const file = event.target.files?.[0]; if (file) uploadCover(file); event.currentTarget.value = ""; }} />{uploading && <span className="mt-2 block text-xs text-[#1371fa]">Cargando imagen...</span>}</Field><Field label="URL de imagen (opcional)"><input value={form.cover_image || ""} onChange={(event) => set("cover_image", event.target.value)} /></Field><Field label="Texto alternativo de portada"><input value={form.cover_alt || ""} onChange={(event) => set("cover_alt", event.target.value)} placeholder="Describe la imagen para buscadores" /></Field><Field label="Título SEO"><input value={form.seo_title || ""} onChange={(event) => set("seo_title", event.target.value)} /></Field><Field label="URL imagen Open Graph"><input value={form.og_image || ""} onChange={(event) => set("og_image", event.target.value)} placeholder="Imagen para compartir en redes" /></Field></div><Field label="Descripción SEO"><textarea value={form.seo_description || ""} onChange={(event) => set("seo_description", event.target.value)} rows={3} /></Field></details><button disabled={busy || uploading} className="mt-7 rounded-full bg-[#0037ff] px-7 py-3 font-semibold text-white shadow-[0_8px_18px_rgba(0,55,255,.22)] disabled:opacity-60">{busy ? "Guardando..." : form.status === "published" ? "Publicar entrada" : "Guardar borrador"}</button></form>{previewOpen && <AdminPostPreview form={form} close={() => setPreviewOpen(false)} />}</>;
  return <section className="mt-7"><div className="flex flex-wrap items-end justify-between gap-4"><p className="max-w-2xl text-[1.02rem] leading-7 text-[#8d98aa]">Historial de entradas: abre cualquiera para editarla, guardarla como borrador o volver a publicarla.</p><button onClick={newPost} className="rounded-full bg-[#0037ff] px-6 py-3 font-semibold text-white shadow-[0_8px_18px_rgba(0,55,255,.24)]">＋ Crear entrada</button></div><div className="mt-5 grid gap-4 sm:grid-cols-[minmax(0,1fr)_12rem]"><div className="flex items-center gap-3 rounded-xl border border-[#dfe5ef] bg-white px-4 py-3 text-[#1371fa] shadow-sm">⌕ <span className="text-sm text-[#a0a8b5]">Buscar entrada</span></div><div className="rounded-xl border border-[#dfe5ef] bg-white px-4 py-3 text-sm text-[#8e96a5] shadow-sm">Todas las categorías</div></div><div className="mt-5 overflow-hidden rounded-2xl border border-white bg-white shadow-[0_5px_18px_rgba(61,91,138,.09)]">{posts.length === 0 ? <div className="grid min-h-72 place-items-center text-center"><div><div className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-[#e7f0ff] text-3xl">✦</div><p className="mt-4 font-medium text-[#8994a6]">Aún no hay entradas para mostrar</p><button onClick={newPost} className="mt-4 rounded-full bg-[#0037ff] px-5 py-2.5 text-sm font-semibold text-white">Crear primera entrada</button></div></div> : posts.map((post) => <button key={post.id} onClick={() => edit(post)} className="flex w-full items-center gap-5 border-b border-[#edf0f4] px-6 py-5 text-left last:border-0 hover:bg-[#f8fbff]"><span className="grid h-11 w-11 place-items-center rounded-xl bg-[#e8f1ff] text-[#1371fa]">▤</span><span className="min-w-0 flex-1"><span className="block truncate font-semibold text-[#505766]">{post.title}</span><span className="mt-1 block text-sm text-[#9aa3b1]">{post.category} · {post.status === "published" ? "Publicado" : "Borrador"} · {formatEntryDate(post.published_at || post.updated_at)}</span></span><span className="text-sm font-semibold text-[#1371fa]">Editar</span></button>)}</div></section>;
}
